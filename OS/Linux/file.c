...
inline fn void fs_fwrite(File *file, String8 str) {
  if (fs_fresize(file, str.size)) { file->prop.size = str.size; }
  memZero(file->content + str.size, ClampBot(0, (isize)file->prop.size - (isize)str.size));
  (void)memCopy(file->content, str.str, str.size);
}
