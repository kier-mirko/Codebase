...
fn String8 os_file_read(Arena *arena, String8 filepath) 
{
  ...
  if (fd >= 0) 
  {
    ...
    {
      result = { .str = (U8 *)make(arena, U8, file_stat.st_size) };
      result.size = read(fd, result.str, file_stat.st_size);
    }
  }
...
fn B32 
os_file_write_list(String8 filepath, String8List content) 
{
  B32 result = 1;
  String8Node *start = content.first++;
  ISZ fd = os_file_write(filepath, start->value);
  if (fd >= 0) 
  {
...
fn FileProperties 
os_file_get_properties(String8 filepath) 
{
...
fn B32 fs_fileWrite(File *file, String8 content) {
  return write(file->descriptor, content.str, content.size) != (ISZ)content.size;
}
...
inline Thread os_lnx_thread_spawn(void *(*thread_main)(void *)) {
  return os_lnx_thread_spawnArgs(thread_main, 0);
}
