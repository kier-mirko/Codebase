...
fn Viewport openglViewport(String8 name, USZ initial_width,
  ...
  String8 glx_exts = str8_from_cstr((char *)
  ...
  if (!opengl_isExtensionSupported(glx_exts, Strlit("GLX_ARB_create_context"))) {
    viewport.opengl_context = glXCreatemakeContext(viewport.xdisplay, bestFbc,
...
B32 viewportShouldClose(Viewport *viewport) {
  XEvent event = {0};
...
B32 viewportSetIcon(Arena *arena, Viewport *viewport, String8 path) {
  USZ head = arena->head;
...
B32 isViewportFullscreen(Viewport *viewport) {
  viewport->xatom_state = XInternAtom(viewport->xdisplay, "_NET_WM_STATE", 0);
  viewport->xatom_state_fullscreen = XInternAtom(viewport->xdisplay,
                                                 "_NET_WM_STATE_FULLSCREEN", 0);
...
B32 isViewportFocused(Viewport *viewport) {
...
fn B32 opengl_isExtensionSupported(String8 ext_list, String8 extension) {
  ...
  for (USZ start = 0, terminator; ;
       start = terminator, ext_list = str8_postfix(ext_list, terminator)) {
    USZ where = str8_find_first_substr(ext_list, extension);
    if (!where) {
      break;
    }
